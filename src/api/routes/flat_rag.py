...
router = APIRouter(prefix="/flat-rag", tags=["Flat RAG"])
...
async def classify_single(request: Request, query: str = Query(..., description="Activity to classify")):
    ...
    logger.info(f"📩 [RAG Flat] classify_one: {query}")
    try:
        async with get_llm_client() as client:
            classifier = RAGFlatClassifier(request.app.state.db, client)
...
async def classify_batch(request: Request, req: BatchActivityRequest):
    logger = logging.getLogger(__name__)
    logger.info(f"📦 [RAG Flat] classify_batch: {len(req.queries)} queries")
    try:
        async with get_llm_client() as client:
            classifier = RAGFlatClassifier(request.app.state.db, client)
